...
export interface EngagementInsightsResponse {
  ...
  /** False for external roles (EC/EV). Gates internal-only UI such as the AI chat panel. */
  ...
  /** Pending AI draft awaiting approval. Internal viewers only; null for externals. */
  insightsSummaryDraft: { content: string; generatedAt: string } | null
  /**
   * True when the published summary was generated from a data snapshot that has since changed.
   * Internal viewers only — a stale summary is flagged, never deleted, so the client keeps
   * seeing approved text and exports always carry a summary.
   */
  insightsSummaryStale: boolean
  insightsSummaryPublishedAt: string | null
 ...
 */
/**
 * Computes the full insights payload. Extracted from the GET handler so background jobs
 * (e.g. the daily AI summary cron) can reuse it without an HTTP request or a user session —
 * callers are responsible for having already authorized access.
 */
export async function computeEngagementInsights(
  projectId: string,
  firmId: string,
  isExternalPersona: boolean,
): Promise<EngagementInsightsResponse> {
  {
    const ctx = { firmId, projectId }
    ...
    const summaryRecord = readInsightsSummary(engagement?.settings as Record<string, unknown> | null)

    const response: EngagementInsightsResponse = {
      insightsConfig,
      isInternalViewer: !isExternalPersona,
      unansweredThreads,
      documentsDueSoon,
      engagementDueDate,
      ...
      insightsSummary: summaryRecord.text,
      // Only an unreviewed draft is surfaced, and only to internal viewers. Approved drafts have
      // already been promoted to `text`; dismissed ones must not be re-prompted.
      insightsSummaryDraft:
        !isExternalPersona && summaryRecord.draft?.status === 'pending_review'
          ? { content: summaryRecord.draft.text, generatedAt: summaryRecord.draft.generatedAt }
          : null,
      // Filled in below — needs the assembled response to compute the current fingerprint.
      insightsSummaryStale: false,
      // Dates the published summary; safe for externals since it accompanies text they already see.
      insightsSummaryPublishedAt: summaryRecord.publishedAt,
    ...
    // Compare the snapshot the published summary described against the current one. A mismatch
    // means the engagement has moved on, so the text is flagged — never removed.
    if (!isExternalPersona && response.insightsSummary && summaryRecord.fingerprint) {
      const { fingerprintInsights } = await import('@/lib/ai/engagement-summary')
      response.insightsSummaryStale = fingerprintInsights(response) !== summaryRecord.fingerprint
    }
    ...
    if (isExternalPersona) {
      return {
        ...
        // Unapproved AI text must never reach a client; staleness is an internal signal.
        // publishedAt passes through — it dates text the client can already see.
