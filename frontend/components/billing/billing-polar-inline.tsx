...
/** Polar wordmark-style mark for inline billing UI (decorative; name is in the adjacent link). */
export function PolarBillingLogo({ className }: { className?: string }) {
    return (
        <svg
            viewBox="0 0 29 29"
            fill="none"
...
/** Logo, linked “Polar” label, and help tooltip — use after copy like “Managed by our billing partner”. */
export function BillingPolarExplainInline({ className }: { className?: string }) {
    return (
        <span className={cn('inline-flex items-center gap-1.5 align-middle', className)}>
            <PolarBillingLogo className="h-[1em] w-[1em] min-h-3.5 min-w-3.5 translate-y-[0.04em]" />
            <Link
