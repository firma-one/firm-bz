...
import { DemoCalendarEngagement } from '@/lib/demo/demo-calendar-data'

interface DemoCalendarSidebarProps {
...
/** Static counterpart to calendar-sidebar.tsx. */
export function DemoCalendarSidebar({
    engagements,
    visibleEngagementIds,
    onToggle,
    onSelectAll,
    onSelectNone,
}: DemoCalendarSidebarProps) {
    const groupedByClient = useMemo(() => {
        const groups = new Map<string, { clientName: string; engagements: DemoCalendarEngagement[] }>()
        for (const e of engagements) {
            const existing = groups.get(e.clientId)
            if (existing) existing.engagements.push(e)
            else groups.set(e.clientId, { clientName: e.clientName, engagements: [e] })
        }
