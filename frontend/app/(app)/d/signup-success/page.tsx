...
export default function SignupSuccessPage() {
    ...
    useEffect(() => {
        async function load() {
            // Get first name from JWT user metadata — works for both OTP and OAuth
            const { data: { user } } = await supabase.auth.getUser()
            if (user) {
                const meta = user.user_metadata ?? {}
                // OTP flow stores first_name; OAuth (Google/Microsoft) stores full_name or name
                const name: string =
                    meta.first_name ||
    ...
    return (
        <div className={`relative min-h-screen w-full max-w-[100vw] overflow-x-hidden overflow-y-hidden bg-[#f0edee] text-[#1b1b1d] selection:bg-[#72ff70]/40 selection:text-[#002203] md:h-[100dvh] md:min-h-0 md:overflow-hidden ${B}`}>
            <div className="pointer-events-none fixed -bottom-40 -left-40 z-0 h-96 w-96 rounded-full bg-[#72ff70]/5 blur-[120px]" aria-hidden />
            <div className="pointer-events-none fixed -right-40 -top-40 z-0 h-96 w-96 rounded-full bg-[#dde1ff]/5 blur-[120px]" aria-hidden />
            ...
            <main className="relative z-10 flex min-h-screen w-full min-w-0 max-w-[100vw] flex-col overflow-x-hidden md:h-full md:min-h-0 md:max-h-full md:flex-row md:overflow-hidden">
                ...
                <section className="relative flex min-h-[48vh] w-full min-w-0 flex-col justify-center bg-[#141c2a] md:h-full md:min-h-0 md:max-h-full md:w-1/2 md:overflow-y-auto md:overflow-x-hidden lg:w-3/5">
                    <div className="absolute inset-0 z-0">
                        ...
                        <img alt="" src={KINETIC_AUTH_HERO_IMAGE} className="h-full w-full object-cover opacity-40 mix-blend-luminosity" />
                        <div className="absolute inset-0 bg-gradient-to-t from-[#141c2a] via-[#141c2a]/35 to-[#141c2a]/60" />
                    </div>

                    <Link href="/" className="group absolute left-8 top-8 z-20 transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] md:left-12 md:top-12" aria-label={`${BRAND_NAME} home`}>
                    ...
                    <div className="relative z-10 mx-auto flex w-full max-w-xl flex-col px-8 py-16 md:px-14 md:py-20 lg:px-16 lg:py-24">
                        <div className="mb-8">
                            <span className={`inline-block text-xs font-bold uppercase tracking-[0.18em] text-[#72ff70] md:text-sm ${H}`}>
                                {KINETIC_LANDING_HERO_BADGE}
                            </span>
                            <h1 className={`mt-4 text-3xl font-bold leading-[1.05] tracking-tighter text-white sm:text-4xl md:text-5xl lg:text-[2.75rem] xl:text-5xl ${H}`}>
                                Turn Your{' '}
                                ...
                                into a <span className="text-[#72ff70]">Professional Client Portal</span>
                            </h1>
                        </div>
                        <p className={`mb-10 max-w-xl text-sm leading-relaxed text-white/75 md:text-base ${B}`}>{KINETIC_HERO_LEAD}</p>
                        <div className="grid grid-cols-1 gap-6 border-t border-white/10 pt-8 sm:grid-cols-2 sm:gap-8">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <span className={`block text-xs font-medium uppercase tracking-widest text-white ${H}`}>Non-custodial</span>
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <span className={`block text-xs font-medium uppercase tracking-widest text-white ${H}`}>Delivery you control</span>
                                    <span className={`mt-1 block text-sm text-white/60 ${B}`}>Revoke-on-close discipline for your IP.</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {/* Right: success card */}
                <section className={`flex min-h-0 w-full min-w-0 flex-col justify-center overflow-x-hidden px-5 py-10 md:h-full md:max-h-full md:w-1/2 md:overflow-hidden md:px-10 md:py-12 lg:w-2/5 lg:px-12 ${B}`}>
                    <div className="mx-auto flex w-full min-w-0 max-w-md flex-col md:h-full md:min-h-0 md:max-h-full">
                            ...
                            <div className="flex min-h-0 min-w-0 flex-1 flex-col justify-center py-8">
                                <div className="max-h-full w-full min-h-0 overflow-y-auto overflow-x-hidden overscroll-y-contain text-left [scrollbar-gutter:stable]">
                                    <div className="mt-8 min-w-0">
                                        {ready
                                            ? <SignupSuccess firstName={firstName} navTarget={navTarget} />
                                            : <div className="py-8 text-[#45474c]">Loading&hellip;</div>
                                        }
                                    </div>
                                </div>
                            </div>
                        ...
                        <footer className="mt-8 flex flex-col items-center justify-between gap-4 text-[10px] uppercase tracking-widest text-[#45474c]/60 sm:mt-10 sm:flex-row sm:gap-6">
                            <div className="flex flex-wrap justify-center gap-6 sm:justify-start">
                                <Link href="/privacy" className={`transition-colors hover:text-[#1b1b1d] ${H}`}>Privacy</Link>
                                <Link href="/terms" className={`transition-colors hover:text-[#1b1b1d] ${H}`}>Terms</Link>
                                <Link href="/contact" className={`transition-colors hover:text-[#1b1b1d] ${H}`}>Support</Link>
                            </div>
                            <div className={`text-[#45474c]/40 ${H}`}>© {new Date().getFullYear()} {BRAND_NAME}.</div>
                        </footer>
                    </div>
                </section>
            </main>
